...
module.exports = {
    ...
    createWeapon: (req, res) => {
        ...
        .then(dbRes => res.status(200).send(dbRes[0]))
        .catch(err => {
            console.log(`There was an error with createWeapon`, err.message);
            res.status(500).send(err);
        })
    },
    ...
    getFightersWeapons: (req, res) => {
        ...
        .then(dbRes => res.status(200).send(dbRes[0]))
        .catch(err => {
            console.log(`There is something wrong with getFightersWeapons`, err.message);
            res.status(500).send(err);
        });
    },
    ...
    deleteWeapon: (req, res) => {
        ...
        sequelize.query(`
            DELETE FROM weapons
                WHERE id = ${id};
        `)
        .then(dbRes => res.stats(200).send(dbRes[0]))
        .catch(err => {
            console.log(`There is an error with deleteWeapon`, err.message);
            res.status(500).send(err);
        });
    }
